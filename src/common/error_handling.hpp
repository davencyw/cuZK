...
class ComputationError : public std::runtime_error {
 public:
  explicit ComputationError(const std::string& message) : std::runtime_error(message) {}
};
...
inline void validate_range(size_t value, size_t min_val, size_t max_val, const std::string& param_name) {
  if (value < min_val || value > max_val) {
    throw ValidationError(param_name + " must be between " + std::to_string(min_val) + 
                         " and " + std::to_string(max_val) + ", got " + std::to_string(value));
  }
}

inline void validate_index(size_t index, size_t max_index, const std::string& context) {
  if (index >= max_index) {
...
#define CUDA_CHECK_THROW(call) \
  CUDA_CHECK_BASE(call, throw ComputationError("CUDA error at " + std::string(__FILE__) + ":" + \
                        std::to_string(__LINE__) + " - " + cudaGetErrorString(error)))

#define CUDA_CHECK_RETURN(call) \
  CUDA_CHECK_BASE(call, return false)

#define CUDA_CHECK_VOID(call) \
  CUDA_CHECK_BASE(call, (void)0)
...
#define CUDA_KERNEL_CHECK() \
  CUDA_CHECK_BASE(cudaGetLastError(), return false)
