...
protected:
  ...
  std::vector<FieldElement> generate_test_leaves(size_t count,
                                                 uint64_t seed = 12345) {
  ...
  for (size_t arity : arities) {
    for (size_t leaf_count : leaf_counts) {
      ...
      EXPECT_EQ(cpu_tree.get_root_hash(), cuda_tree.get_root_hash())
          << "Root hashes differ for arity=" << arity
          << ", leaves=" << leaf_count;
...
TEST_F(MerkleTreeCUDATest, BatchTreeBuildingTest) {
  ...
  auto batch_leaves = CudaMerkleUtils::generate_batch_test_leaves(
      num_trees, leaves_per_tree, 999);

  std::vector<CudaNaryMerkleTree> trees;
  ASSERT_TRUE(
      CudaNaryMerkleTree::build_batch_trees(batch_leaves, trees, config));
...
TEST_F(MerkleTreeCUDATest, PerformanceComparisonTest) {
  ...
  auto cpu_time = std::chrono::duration_cast<std::chrono::microseconds>(
      end_cpu - start_cpu);
  auto cuda_time = std::chrono::duration_cast<std::chrono::microseconds>(
      end_cuda - start_cuda);
  ...
  bool cpu_result = cpu_tree.verify_batch_proofs(cpu_proofs, proof_leaves,
                                                 cpu_tree.get_root_hash());
  ...
  auto cpu_verify_time = std::chrono::duration_cast<std::chrono::microseconds>(
      end_cpu - start_cpu);
  auto cuda_verify_time = std::chrono::duration_cast<std::chrono::microseconds>(
      end_cuda - start_cuda);

  std::cout << "Performance Comparison (Batch Proof Verification):"
            << std::endl;
  ...
  if (cuda_verify_time.count() > 0) {
    double verify_speedup =
        static_cast<double>(cpu_verify_time.count()) / cuda_verify_time.count();
...
TEST_F(MerkleTreeCUDATest, TreeHeightDebuggingTest) {
  ...
  for (size_t arity : arities) {
    for (size_t leaf_count : leaf_counts) {
      std::cout << "\nArity: " << arity << ", Leaves: " << leaf_count
                << std::endl;

      // Method 1: CPU static method
      size_t cpu_static_height =
          NaryMerkleTree::calculate_tree_height(leaf_count, arity);
      ...
      std::cout << "  CPU proof gen style (ceil): " << proof_gen_height
                << std::endl;
      ...
      std::cout << "  CUDA actual tree: " << cuda_tree.get_tree_height()
                << std::endl;
...
TEST_F(MerkleTreeCUDATest, BinaryTreeSpecificTest) {
  ...
  for (size_t leaf_count : leaf_counts) {
    std::cout << "\nTesting binary tree with " << leaf_count
              << " leaves:" << std::endl;
    ...
    std::cout << "  CPU tree height: " << cpu_tree.get_tree_height()
              << std::endl;
    std::cout << "  CUDA tree height: " << cuda_tree.get_tree_height()
              << std::endl;
    std::cout << "  CPU root: "
              << cpu_tree.get_root_hash().to_hex().substr(0, 16) << "..."
              << std::endl;
    std::cout << "  CUDA root: "
              << cuda_tree.get_root_hash().to_hex().substr(0, 16) << "..."
              << std::endl;
    ...
    std::cout << "  Result: "
              << (cuda_tree.get_root_hash() == cpu_tree.get_root_hash()
                      ? "PASS"
                      : "FAIL")
              << std::endl;
  }
...
TEST_F(MerkleTreeCUDATest, ProofGenerationAfterHeightFix) {
  ...
  std::cout << "  Roots match: "
            << (cpu_tree.get_root_hash() == cuda_tree.get_root_hash() ? "YES"
                                                                      : "NO")
            << std::endl;
  ...
  for (size_t idx : test_indices) {
    ...
    ASSERT_TRUE(cpu_proof.has_value())
        << "CPU proof generation failed for index " << idx;
    ASSERT_TRUE(cuda_proof.has_value())
        << "CUDA proof generation failed for index " << idx;

    std::cout << "  CPU proof levels: " << cpu_proof->path.size() << std::endl;
    std::cout << "  CUDA proof levels: " << cuda_proof->path.size()
              << std::endl;
    ...
    std::cout << "  Proof verification: "
              << (cuda_tree.verify_proof(*cuda_proof, leaves[idx]) ? "PASS"
                                                                   : "FAIL")
              << std::endl;
  }
}
...
TEST_F(MerkleTreeCUDATest, ComprehensiveValidationTest) {
  ...
  std::vector<TestCase> test_cases = {
      {2, 4, "Binary tree with 4 leaves (edge case)"},
  ...
  for (const auto &test_case : test_cases) {
    ...
    std::vector<FieldElement> leaves =
        generate_test_leaves(test_case.leaves, 42);
    ...
    std::cout << "  Heights match: "
              << (cpu_tree.get_tree_height() == cuda_tree.get_tree_height()
                      ? "YES"
                      : "NO")
              << std::endl;
    std::cout << "  Roots match: "
    ...
    if (test_case.leaves > 0) {
      ...
      if (cpu_proof && cuda_proof) {
        std::cout << "  Proof structure: CPU=" << cpu_proof->path.size()
                  << " levels, CUDA=" << cuda_proof->path.size() << " levels"
                  << std::endl;

        EXPECT_EQ(cpu_proof->path.size(), cuda_proof->path.size())
            << "Proof path sizes differ for " << test_case.description;

        bool cpu_verify = cpu_tree.verify_proof(*cpu_proof, leaves[test_index],
                                                cpu_tree.get_root_hash());
        bool cuda_verify =
            cuda_tree.verify_proof(*cuda_proof, leaves[test_index]);
        bool cross_verify = cpu_tree.verify_proof(
            *cpu_proof, leaves[test_index], cuda_tree.get_root_hash());

        std::cout << "  Proof verification: CPU="
                  << (cpu_verify ? "PASS" : "FAIL")
                  << ", CUDA=" << (cuda_verify ? "PASS" : "FAIL")
                  << ", Cross=" << (cross_verify ? "PASS" : "FAIL")
                  << std::endl;

        EXPECT_TRUE(cpu_verify)
            << "CPU proof verification failed for " << test_case.description;
        EXPECT_TRUE(cuda_verify)
            << "CUDA proof verification failed for " << test_case.description;
        EXPECT_TRUE(cross_verify)
            << "Cross proof verification failed for " << test_case.description;
      }
    }
    ...
    EXPECT_TRUE(overall_success)
        << "Overall validation failed for " << test_case.description;
  }
}
...
TEST_F(MerkleTreeCUDATest, DetailedLevelDebuggingTest) {
  ...
  for (size_t i = 0; i < leaves.size(); ++i) {
    std::cout << "  Leaf[" << i << "]: " << leaves[i].to_hex().substr(0, 16)
              << "..." << std::endl;
  }
  ...
  std::cout << "\nCUDA tree levels (" << cuda_levels.size()
            << " levels):" << std::endl;
  for (size_t level = 0; level < cuda_levels.size(); ++level) {
    ...
    for (size_t i = 0; i < cuda_levels[level].size() && i < 8; ++i) {
      std::cout << "    [" << i
                << "]: " << cuda_levels[level][i].to_hex().substr(0, 16)
                << "..." << std::endl;
    }
  }
  ...
  std::cout << "  Empty hash: " << empty_hash.to_hex().substr(0, 16) << "..."
            << std::endl;
  ...
  std::cout << "  Level 0 (padded leaves, " << current_level.size()
            << " elements):" << std::endl;
  for (size_t i = 0; i < current_level.size(); ++i) {
    std::cout << "    [" << i
              << "]: " << current_level[i].to_hex().substr(0, 16) << "..."
              << std::endl;
  }
    ...
    for (size_t i = 0; i < current_level.size(); ++i) {
      if (current_level[i] != cuda_levels[0][i]) {
        ...
        std::cout << "    MISMATCH at [" << i
                  ...
                  << "..., CUDA=" << cuda_levels[0][i].to_hex().substr(0, 16)
                  << "..." << std::endl;
      }
    }
  } else {
  ...
  std::cout << "  Level 0 match: " << (level0_match ? "YES" : "NO")
            << std::endl;
  ...
  while (current_level.size() > 1) {
    ...
    std::cout << "  Building level " << level_num << " from "
              << current_level.size() << " inputs:" << std::endl;

    for (size_t i = 0; i < current_level.size(); i += config.arity) {
      std::vector<FieldElement> children;

      // Collect children
      for (size_t j = 0; j < config.arity && i + j < current_level.size();
           ++j) {
      ...
      for (size_t k = 0; k < children.size(); ++k) {
        std::cout << children[k].to_hex().substr(0, 8)
                  << (k < children.size() - 1 ? ", " : "");
      }

      // Hash using the same method as CPU
      FieldElement parent_hash =
          Poseidon::PoseidonHash::hash_multiple(children);
      next_level.push_back(parent_hash);

      std::cout << " -> " << parent_hash.to_hex().substr(0, 16) << "..."
                << std::endl;
    }
    ...
    if (cuda_levels.size() > level_num &&
        cuda_levels[level_num].size() == next_level.size()) {
      for (size_t i = 0; i < next_level.size(); ++i) {
        if (next_level[i] != cuda_levels[level_num][i]) {
          ...
          std::cout << "    LEVEL " << level_num << " MISMATCH at [" << i
                    ...
                    << "..., CUDA="
                    << cuda_levels[level_num][i].to_hex().substr(0, 16) << "..."
                    << std::endl;
        }
      }
    } else {
      ...
      std::cout << "    LEVEL " << level_num
                << " SIZE MISMATCH: CPU=" << next_level.size();
      if (cuda_levels.size() > level_num) {
    ...
    std::cout << "  Level " << level_num
              << " match: " << (level_match ? "YES" : "NO") << std::endl;
  ...
  std::cout << "  Manual CPU root: " << current_level[0].to_hex().substr(0, 16)
            << "..." << std::endl;
  std::cout << "  CPU tree root: "
            << cpu_tree.get_root_hash().to_hex().substr(0, 16) << "..."
            << std::endl;
  std::cout << "  CUDA tree root: "
            << cuda_tree.get_root_hash().to_hex().substr(0, 16) << "..."
            << std::endl;
  std::cout << "  Manual vs CPU: "
            << (current_level[0] == cpu_tree.get_root_hash() ? "MATCH"
                                                             : "DIFFER")
            << std::endl;
  std::cout << "  Manual vs CUDA: "
            << (current_level[0] == cuda_tree.get_root_hash() ? "MATCH"
                                                              : "DIFFER")
            << std::endl;
