...
struct MerkleTreeConfig {
  ...
  static constexpr size_t DEFAULT_TREE_HEIGHT =
      20; // Default height for empty tree

  size_t arity;
  size_t tree_height;

  explicit MerkleTreeConfig(size_t arity = DEFAULT_ARITY,
                            size_t tree_height = DEFAULT_TREE_HEIGHT)
...
struct MerkleNode {
  ...
  explicit MerkleNode(const FieldElement &value, bool leaf = false)
      : hash(value), is_leaf(leaf) {}
};
...
private:
  ...
  build_tree_bottom_up(const std::vector<FieldElement> &leaves);

  FieldElement
  compute_internal_hash(const std::vector<FieldElement> &children_hashes) const;
...
public:
  ...
  bool verify_proof(const MerkleProof &proof, const FieldElement &leaf_value,
                    const FieldElement &root_hash) const;

  // Batch operations
  std::vector<MerkleProof>
  generate_batch_proofs(const std::vector<size_t> &indices) const;
...
private:
  void print_tree_recursive(const std::shared_ptr<MerkleNode> &node,
                            size_t level, const std::string &prefix) const;
};
