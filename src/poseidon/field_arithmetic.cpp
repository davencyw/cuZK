...
const FieldElement MODULUS =
    FieldElement(0x43e1f593f0000001ULL, 0x2833e84879b97091ULL,
...
FieldElement::FieldElement() { std::memset(limbs, 0, sizeof(limbs)); }
...
std::string FieldElement::to_dec() const {
  ...
  while (!temp.is_zero()) {
    ...
    for (int i = 3; i >= 0; --i) {
      ...
      __uint128_t current =
          (static_cast<__uint128_t>(remainder) << 64) | temp.limbs[i];
...
FieldElement FieldElement::random() { return FieldArithmetic::random(); }

bool FieldElement::is_zero() const {
  return limbs[0] == 0 && limbs[1] == 0 && limbs[2] == 0 && limbs[3] == 0;
}

void FieldElement::set_zero() { std::memset(limbs, 0, sizeof(limbs)); }
...
void add(const FieldElement &a, const FieldElement &b, FieldElement &result) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    __uint128_t sum =
        (__uint128_t)a.limbs[i] + (__uint128_t)b.limbs[i] + (__uint128_t)carry;
...
void subtract(const FieldElement &a, const FieldElement &b,
  ...
  if (a < b) {
    ...
    uint64_t carry = 0;
    for (int i = 0; i < 4; ++i) {
      __uint128_t sum = (__uint128_t)a.limbs[i] +
                        (__uint128_t)FieldConstants::MODULUS.limbs[i] +
                        (__uint128_t)carry;
...
void reduce_512(const uint64_t product[8], FieldElement &result) {
  ...
  static const FieldElement k = FieldElement::from_hex(
      "0x0e0a77c19a07df2f666ea36f7879462e36fc76959f60cd29ac96341c4ffffffb");
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    for (int j = 0; j < 4; ++j) {
      __uint128_t prod =
          (__uint128_t)high.limbs[i] * k.limbs[j] + mult_product[i + j] + carry;
  ...
  // We need to reduce this 512-bit result, but we can't call reduce_512
  // recursively Instead, we'll apply the reduction algorithm iteratively

  // Extract the low and high parts of mult_product
  FieldElement mult_low(mult_product[0], mult_product[1], mult_product[2],
                        mult_product[3]);
  FieldElement mult_high(mult_product[4], mult_product[5], mult_product[6],
                         mult_product[7]);
  ...
  // But we'll do this only once to avoid infinite recursion, and then rely on
  // final reduce()
  if (!mult_high.is_zero()) {
    ...
    for (int i = 0; i < 4; ++i) {
      uint64_t carry = 0;
      for (int j = 0; j < 4; ++j) {
        __uint128_t prod = (__uint128_t)mult_high.limbs[i] * k.limbs[j] +
                           mult2_product[i + j] + carry;
    ...
    FieldElement mult2_low(mult2_product[0], mult2_product[1], mult2_product[2],
                           mult2_product[3]);
