...
TEST_F(FieldAccumulationTest, CoefficientAccumulation) {
    ...
    EXPECT_EQ(result.to_dec(), "21888242871839275222246405745257275088548364400416034343698204186575808495616");
    
    // Subtract 3
    result += (FieldConstants::ZERO - FieldElement(3));
    EXPECT_EQ(result.to_dec(), "21888242871839275222246405745257275088548364400416034343698204186575808495613");
...
TEST_F(FieldAccumulationTest, DirectComputation) {
    ...
    FieldElement direct = FieldElement(1) - FieldElement(2) - FieldElement(3) + FieldElement(4);
    
    EXPECT_TRUE(direct.is_zero()) << "Direct computation should be zero: " << direct.to_dec();
