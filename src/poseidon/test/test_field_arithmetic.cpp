...
protected:
  ...
  FieldElement createTestValue(uint64_t v0, uint64_t v1 = 0, uint64_t v2 = 0,
                               uint64_t v3 = 0) {
    FieldElement result;
    result.limbs[0] = v0;
    result.limbs[1] = v1;
...
TEST_F(FieldArithmeticTest, PowerFunction) {
  ...
  // function
  auto simple_power = [](const FieldElement &base, uint64_t exp,
                         FieldElement &result) {
    if (exp == 0) {
...
TEST_F(FieldArithmeticTest, Negation) {
  ...
  if (FieldConstants::ZERO.is_zero()) {
    ...
    FieldArithmetic::subtract(FieldConstants::MODULUS, FieldConstants::ZERO,
                              neg_zero);
  }
...
TEST_F(FieldArithmeticTest, DecimalConversion) {
  ...
  EXPECT_EQ(modulus_dec, "21888242871839275222246405745257275088548364400416034"
                         "343698204186575808495617");

  // Test consistency - convert back and forth shouldn't change hex
  // representation
  FieldElement test_val =
      createTestValue(0xDEADBEEFCAFEBABEULL, 0x1234567890ABCDEFULL, 0, 0);
...
TEST_F(FieldArithmeticTest, LargeNumberArithmetic) {
  ...
  FieldElement large1 =
      createTestValue(0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0, 0);
  FieldElement large2 =
      createTestValue(0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0, 0);
...
TEST_F(FieldArithmeticTest, SubtractionEdgeCases) {
  ...
  EXPECT_EQ(verification, small)
      << "Modular subtraction should satisfy (a - b) + b = a";
  ...
  EXPECT_EQ(zero_minus_one, modulus_minus_one)
      << "ZERO - ONE should equal MODULUS - ONE";
}
...
TEST_F(FieldArithmeticTest, Reduce512) {
  ...
  // Test case 3: Both high and low parts are non-zero
  {
    ...
    // It compares reduce_512() against multiply(), but multiply() internally
    // calls reduce_512() This makes the test unreliable. The
    // Reduce512MathematicalCorrectness test provides better validation of the
    // mathematical correctness without circular dependencies. EXPECT_EQ(result,
    // expected_result);
  }

  // Test case 4: Large values that definitely need reduction
  {
    uint64_t product[8] = {0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL,
  ...
  {
    FieldElement a =
        createTestValue(0x123456789ABCDEFULL, 0xFEDCBA9876543210ULL, 0, 0);
    FieldElement b =
        createTestValue(0xAABBCCDDEEFF1122ULL, 0x3344556677889900ULL, 0, 0);
...
TEST_F(FieldArithmeticTest, Reduce512EdgeCases) {
  ...
  // Test case 1: Test the 2^256 mod p approximation accuracy
  {
    ...
    EXPECT_EQ(result, exact_two_to_256)
        << "2^256 mod p approximation should be exact";
  }
  ...
  // Test case 4: Test that different representations of the same value give the
  // same result
  {
    ...
    EXPECT_EQ(result1, result2)
        << "Different representations of p + 5 should reduce to the same value";
  ...
  // Test case 5: Test multiple reduction iterations needed
  {
    ...
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Result should be properly reduced even when multiple iterations "
           "needed";
  }
  ...
  // Test case 6: Test the correctness of 2^256 mod p computation
  {
    ...
    for (int power = 1; power <= 5; ++power) {
      ...
      EXPECT_EQ(result, expected)
          << "2^256 * " << power << " mod p should equal " << power
          << " * (2^256 mod p) mod p";
    }
  }
  ...
  // Test case 7: Test boundary conditions near modulus
  {
    ...
    // Expected result: (modulus - 1) + (2^256 mod p) ≡ -1 + (2^256 mod p) (mod
    // p)
    ...
    EXPECT_EQ(result, expected) << "(modulus - 1) + 2^256 should equal "
                                   "(modulus - 1) + (2^256 mod p) mod p";
  }
}
...
TEST_F(FieldArithmeticTest, Reduce512ConsistencyWithModularArithmetic) {
  ...
  // Test case 1: Consistency test - reduce_512 should be mathematically
  // consistent
  {
    ...
    // This is because (a + b) mod p ≠ (a mod p) + (b mod p) when the sum
    // overflows Instead, we test that all results are properly reduced
    ...
    // calls reduce_512() This makes the test unreliable. The
    // Reduce512MathematicalCorrectness test provides better validation of the
    // mathematical correctness without circular dependencies.
    // EXPECT_EQ(result_a, expected_a) << "reduce_512 should match manual
  ...
  // Test case 2: Test with known multiplication results
  {
    ...
    EXPECT_EQ(result, expected)
        << "reduce_512 should give same result as regular multiplication";
  }
  ...
  // Test case 3: Test commutativity through reduce_512
  {
    ...
    FieldElement a =
        createTestValue(0x123456789ABCDEFULL, 0x1111222233334444ULL, 0, 0);
    FieldElement b =
        createTestValue(0xFEDCBA9876543210ULL, 0x5555666677778888ULL, 0, 0);
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      for (int j = 0; j < 4; ++j) {
        __uint128_t prod =
            (__uint128_t)a.limbs[i] * b.limbs[j] + product_ab[i + j] + carry;
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      for (int j = 0; j < 4; ++j) {
        __uint128_t prod =
            (__uint128_t)b.limbs[i] * a.limbs[j] + product_ba[i + j] + carry;
    ...
    EXPECT_EQ(result_ab, result_ba)
        << "reduce_512 should preserve commutativity: reduce_512(a*b) = "
           "reduce_512(b*a)";
  }
}
...
TEST_F(FieldArithmeticTest, Reduce512SpecialValues) {
  ...
  // Test case 1: Powers of 2
  {
    for (int power = 0; power < 8; ++power) {
      ...
      EXPECT_TRUE(result < FieldConstants::MODULUS)
          << "2^" << (64 * power) << " mod p should be properly reduced";
    }
  }
  ...
  // Test case 2: All ones in different positions
  {
    for (int pos = 0; pos < 8; ++pos) {
      ...
      EXPECT_TRUE(result < FieldConstants::MODULUS)
          << "All-ones at position " << pos << " should be properly reduced";
    }
  }

  // Test case 3: Alternating bit patterns
  {
    uint64_t patterns[] = {0xAAAAAAAAAAAAAAAAULL, 0x5555555555555555ULL,
  ...
  // Test case 4: Test exact modulus multiples
  {
    ...
    EXPECT_TRUE(result2 < FieldConstants::MODULUS)
        << "Large value should be properly reduced";
  }
}

TEST_F(FieldArithmeticTest, Reduce512MathematicalCorrectness) {
  // This test verifies the mathematical correctness of the reduce_512
  // implementation by testing the core assumption: 2^256 ≡ ? (mod p) for BN254
  ...
  // Test case 1: Verify the exact value of 2^256 mod p
  {
    ...
    FieldElement four(4);
    EXPECT_NE(two_to_256, four) << "2^256 mod p should NOT equal 4 - this "
                                   "confirms the old approximation was wrong";
  }
  ...
  // Test case 3: Test the shift-by-2 logic for potential overflow
  {
    ...
    TestCase test_cases[] = {
        ...
        {{0, 0, 0, 0x4000000000000000ULL},
         "Overflow potential in highest limb"},
        {{0xC000000000000000ULL, 0xC000000000000000ULL, 0xC000000000000000ULL,
          0xC000000000000000ULL},
         "All limbs with potential overflow"},
    };
    ...
    for (const auto &test_case : test_cases) {
      ...
      EXPECT_TRUE(result < FieldConstants::MODULUS)
          ...
          << "' resulted in unreduced value: " << result.to_hex();
  ...
  // Test case 4: Verify that the function handles the modulus correctly
  {
    ...
    uint64_t product[8] = {FieldConstants::MODULUS.limbs[0],
    ...
    FieldArithmetic::reduce_512(product, result);
    EXPECT_EQ(result, FieldConstants::ZERO)
        << "Input equal to modulus should reduce to zero";

    // Test input that is exactly 2*p (should also reduce to 0)
    uint64_t carry = 0;
    for (int i = 0; i < 4; ++i) {
      __uint128_t doubled =
          (__uint128_t)FieldConstants::MODULUS.limbs[i] * 2 + carry;
    ...
    FieldArithmetic::reduce_512(product, result);
    EXPECT_EQ(result, FieldConstants::ZERO)
        << "Input equal to 2*modulus should reduce to zero";
  }
}
...
TEST_F(FieldArithmeticTest, Reduce512OverflowAndCircularDependencyTests) {
  ...
  // Test case 1: Large high part that would cause massive overflow in
  // multiplication
  {
    // Set high part to maximum values - this would cause overflow when
    // multiplied by 2^256 mod p
    uint64_t product[8] = {
        100,        200,
        300,        400, // Low part (modest values)
        UINT64_MAX, UINT64_MAX,
        UINT64_MAX, UINT64_MAX // High part (maximum values)
    };

    FieldElement result;
    FieldArithmetic::reduce_512(product, result);

    // The result should be properly reduced and not cause any overflow or
    // infinite recursion
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Large high part should not cause overflow";
  ...
  // Test case 2: Values that would trigger circular dependency in buggy
  // implementation
  {
    // These values would cause the multiply function to be called with large
    // arguments In the buggy version, this would lead to infinite recursion
    uint64_t product[8] = {0x123456789abcdef0ULL, 0xfedcba9876543210ULL,
    ...
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Should not cause circular dependency";
  }
  ...
  // Test case 3: Maximum possible 512-bit input
  {
    ...
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Maximum 512-bit value should be handled correctly";
  }

  // Test case 4: Specific pattern that would cause overflow in multiplication
  {
    // High part with values that, when multiplied by 2^256 mod p, would
    // overflow 256 bits
    uint64_t product[8] = {
        0,          0,
        0,          0, // Zero low part to isolate high part effects
        1ULL << 60, 1ULL << 61,
        1ULL << 62, 1ULL << 63 // Large powers of 2
    };
    ...
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Large power-of-2 values should not overflow";
  }

  // Test case 5: Alternating pattern to test bit-by-bit processing
  {
    uint64_t product[8] = {0x5555555555555555ULL, 0xaaaaaaaaaaaaaaaaULL,
    ...
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Alternating bit pattern should be handled correctly";
  }
  ...
  // Test case 6: Performance stress test - ensure no exponential complexity
  {
    ...
    EXPECT_LT(duration.count(), 1000)
        << "reduce_512 should not have exponential complexity";
  }

  // Test case 7: Verify consistency across multiple reductions
  {
    uint64_t product[8] = {0x123456789abcdef0ULL,
...
TEST_F(FieldArithmeticTest, Reduce512ExtremeCases) {
  ...
  // Test case 1: "Multiplication bomb" - values that would cause massive
  // overflow
  {
    // High part filled with near-maximum values
    // In the buggy version, multiplying this by 2^256 mod p would overflow
    // spectacularly
    uint64_t product[8] = {1,
                           1,
                           1,
                           1,                     // Small low part
    ...
    // This should complete without crashing, hanging, or producing invalid
    // results
    FieldArithmetic::reduce_512(product, result);

    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Extreme values should not cause system failure";

    // Verify the result is mathematically sensible
    EXPECT_FALSE(result.is_zero())
        << "Non-zero input should produce non-zero result";
  }

  // Test case 2: "Recursion depth bomb" - pattern that maximizes recursion in
  // buggy version
  {
    // This pattern would trigger maximum multiply calls in the buggy
    // implementation
    uint64_t product[8] = {
        0,
        0,
        0,
        0,                     // Zero low part to maximize high part impact
    ...
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "High-bit pattern should not cause infinite recursion";
  }

  // Test case 3: "Computational complexity bomb"
  {
    // Values chosen to maximize the number of 1-bits, which would maximize
    // the computational work in our bit-by-bit algorithm (but should still be
    // fast)
    uint64_t product[8] = {0x5555555555555555ULL, 0xAAAAAAAAAAAAAAAAULL,
                           0x5555555555555555ULL, 0xAAAAAAAAAAAAAAAAULL,
                           0x5555555555555555ULL, 0xAAAAAAAAAAAAAAAAULL,
    ...
    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Complex bit pattern should be handled correctly";
    EXPECT_LT(duration.count(), 10000)
        << "Should complete in reasonable time (< 10ms) even for worst case";
  }

  // Test case 4: Multiple consecutive large reductions
  {
    // Verify that multiple large reductions in a row don't cause cumulative
    // issues
    std::vector<uint64_t> test_patterns[3] = {
        ...
        {0x5A5A5A5A5A5A5A5AULL, 0xA5A5A5A5A5A5A5A5ULL, 0x5A5A5A5A5A5A5A5AULL,
         0xA5A5A5A5A5A5A5A5ULL, 0x5A5A5A5A5A5A5A5AULL, 0xA5A5A5A5A5A5A5A5ULL,
    ...
    for (int pattern = 0; pattern < 3; ++pattern) {
      ...
      EXPECT_TRUE(result < FieldConstants::MODULUS)
          << "Pattern " << pattern << " should be reduced correctly";
    }
  }
  ...
  // Test case 5: Verify no memory corruption with extreme values
  {
    ...
    uint64_t product[8] = {0xDEADBEEFCAFEBABEULL, 0x1234567890ABCDEFULL,
                           0xFEDCBA0987654321ULL, 0xABCDEF1234567890ULL,
                           0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL,
                           0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL};
    ...
    EXPECT_EQ(guard_before, 0xDEADBEEFDEADBEEFULL)
        << "Stack corruption detected before";
    EXPECT_EQ(guard_after, 0xCAFEBABECAFEBABEULL)
        << "Stack corruption detected after";

    EXPECT_TRUE(result < FieldConstants::MODULUS)
        << "Extreme values should not corrupt memory";
  }
}
